...
%
\fuzzchick is a tool coverage-guided, property-based testing (CGPBT) tool that
...
%
Despite being enlighthening, we observed several limitations in the
...
In this work we present \mutagen, a CGPBT framework that builds upon the ideas
behind \fuzzchick.
%
...
one used by \fuzzchick, as well as finding subtle bugs in real-world scenarios.
\end{abstract}
...
codebase, e.g., a data type definition.
%
...
be mutated using type-preserving transformations to produce new ones.
%
...
\fuzzchick relies on execution traces to distinguish which test cases resulted
interesting and are therefore worth mutating --- a prolific technique known as
\emph{coverage-guided fuzzing} and popularized by tools like \emph{AFL}
\cite{afl}.
%
...
%
Notably, we establish several aspects of its implementation that can be
revised. \footnote{Some of these aspects were already identified by the authors
  of \fuzzchick in their original work.}
%
In particular:
%
\begin{inparaenum}
...
\item \emph{the mechanism used to assign a certain mutation energy to each
...
important metric when comparing PBT tools.
...
randomness.
%
On the other hand, scheduling mutations exhaustively eliminates the need of a
heuristic that assigns a given mutation energy to each input test case.
...
times.
%
This way, our tool avoids, for instance, mutating a given number into every
...
%
All in all, our evaluation encompass more than 600 hours of cpu time and suggest
that testing mutants exhaustively can be an appealing technique for finding bugs
...
%
Generic fuzzing tools like \emph{AFL}, \emph{libFuzzer} \citeyearpar{libfuzzer}
or \emph{HonggFuzz} \cite{honggfuzz}, as well as language-specific ones like
Crowbar \cite{dolan2017testing} or \emph{Kelinci} \cite{kersten2017poster} use
execution traces to recognize interesting test cases, e.g, those that exercise
previously undiscovered parts of the target code.
%
...
act over the ``serialized'' of the test cases and know very little about the
structure of the generated data, thus producing syntactically broken mutants
most of the time.
%
...
its corresponding queue, depending on whether it passed or was discarded (i.e.,
it failed the precondition).
%
...
\paragraph{Mutators distribution:}
%
...
%
Hence, \fuzzchick mutators can only be effectively used to transform to shallow
...
%
If that mutated test case turns to be interesting, that subexpression can always
be mutated later.

\paragraph{Enqueuing mutation candidates:}
%
...
\begin{inparaenum}
  ...
  \item for every mutable subexpression, each one of these mutations is
    evaluated \emph{exactly once}.
\end{inparaenum}
%
%% There is a small exception to this rules that we will introduce soon.

This idea is inspired by exhaustive bounded testing tools like SmallCheck
\cite{runciman2008smallcheck} or Korat \cite{boyapati2002korat}, that produce
...
search space based on detecting unevaluated subexpressions --- this is discussed
...
%
Moreover, like in \fuzzchick, the testing loop of our tool automatically filters
...
%
Instead of mutating into every other possible number or character, we resolve in
using a random generator and sample a small number of values from it --- the
precise amount is a tunable parameter of \mutagen.
%
...
interpreter of this engine, and used the reference WebAssembly implementation to
find them via differential testing \cite{mckeeman1998differential}.
%
...
7
& Validator
& Wrong type validation on \texttt{i32} binary opertations \\
\hline
8
& Validator
...
11
& Interpreter
& Wrong intepretation of \texttt{i32.sub} instruction \\
\hline
12
& Interpreter
& Wrong intepretation of \texttt{i32.lt\_u} instruction when operands are equal \\
\hline
13
& Interpreter
& Wrong intepretation of \texttt{i32.shr\_u} instruction \\
\hline
14
& Interpreter
...
To keep things simple here as well, we use a function \texttt{mk\_module} to
creates a stub module which initializes one memory block and exports a single
function.
%
This function is parameterized by the definition of the single function, its
...
implementation interpreter with the provided arguments.
%
Then, the property asserts whether their results are equivalent.
%
\footnote{In our implementation, we additionally set a 20ms timeout to discard
...
Firstly, we focus on the bugs injected in the validator (Fig.
...
hardest-to-find bugs (\#7, \#8, and \#1) are no longer found on every run.
%
...
run.
%
Finding where the best tradeoff lies in cases where the heuristics cause mixed
...
\quickcheck --- roughly 20x and 75x slower when testing \textit{prop\_validator}
and \textit{prop\_interpreter\_i32}, respectively.
%
...
%
\cite{ClaessenDP14} developed an algorithm for generating inputs constrained by
boolean preconditions with almost-uniform distribution.
%
% Luck
%
Later, \cite{LampropoulosGHH17} extends this approach by adding a limited form
of constraint solving controllable by the user in a domain-specific language
called \emph{Luck}.
%
% Deriving good generators
%
Recently, \cite{Lampropoulos2017} proposed a derivation mechanism to obtain
constrained random generators directly by defining their structure using
inductively defined relations in Coq.
...
testing every possible input value of the system under test.
...
of the code; and evaluating the effect of lazy prunning; among others.
%
