...
\acmConference[ESEC/FSE 2022]{The 30th ACM Joint European Software Engineering
Conference and Symposium on the Foundations of Software Engineering}{14 - 18
...
%
Unfortunately, such techniques suffer from severe limitations when used to
produce random test cases satisfying complex invariants, which are often needed
to test properties with sparse preconditions.
...
as cover the search space in a principled manner.   
%
...
%
% Popularized by \quickcheck \cite{ClaessenH00},
%
...
e.g., data type definitions and application public interfaces (APIs).
...
\end{inparaenum}
%
Overcoming these obstacles is important to make CGPT more suitable for testing
real-world software. 
...
%% Validation
We validated our ideas using two case studies which require generating
highly-structured inputs (described in detail in \S \ref{sec:casestudies}): the
Information-Flow Control (IFC) stack machine used by
\citeauthor{lampropoulos2019coverage} in their original work, and an existing
{Web-Assembly} engine of industrial strength written in the Haskell programming
language.
%
...
reference CGPT implementation developed for the Coq proof assistant
...
In this case study we also evaluate the performance versus the overhead of our
tool (and our custom code instrumentation mechanism) by comparing it against
\quickcheck \cite{ClaessenH00}, the most widely used RPBT tool in Haskell.  
%
...
Readers familiar with RBPT will recognize that using \texttt{genTree} to
generate test cases to test \texttt{prop\_insert} with \quickcheck directly does
not work.
%
...
%  depending on whether it passed the property precondition or
% was discarded.
%
...
%, especially when it comes to the mutator's design.
%
%% Moreover, we replicated the Information-Flow Control (IFC) stack machine case
%
% In addition, 
However, we observed a notable limitation after replicating the IFC stack
machine case study bundled with \fuzzchick:
%  and observed a notable limitation:
%
...
% especially considering how expressive the data
% types used to derive the mutators can be.  
%
...
% We now introduce \mutagen, our revised CGPT approach designed to tackle the
% limitations of CGPT mentioned so far.

% of CGPT to make it practical in real-world scenarios.
...
% In contrast to CGPT and many other popular fuzzing tools,
...
\texttt{mutate}(C_i \, x_1 \, x_2 \, \mydots) = 
  \mathtt{mut_{r}}(C_i \, x_1 \, x_2 \, \mydots) \cup
  \mathtt{mut_{s}}(C_i \, x_1 \, x_2 \, \mydots) && 
      % \setminus
      %  \{ (C_i \, x_1 \, x_2 \, \mydots) \} &&
\end{flalign*}
\vspace{-7pt}
...
\vspace{-10pt}
\begin{flalign*}
\mathtt{mut_{r}}(C_i \, x_1 \, x_2 \, \mydots) =  
  \{ 
    \mathtt{PURE}\, x_k
    \;|\;
...
\mathtt{mut_{s}}(C_i \, x_1 \, x_2 \, \mydots) \!=\! 
  \Vast\{ 
    \mathtt{PURE}(C_j \, x_1^\prime \, x_2^\prime \, \mydots)
    \Vast|\,
    \begin{aligned}
      & C_j \!\in\! \{ C_1, C_2, \mydots \} \\
      & x_k^\prime \!\in\! \overline{\mathtt{filter}}(t_k^j, \{x_1, \, x_2, \, \mydots \}) \\  
...
\vspace{-10pt}
\begin{flalign*}
\overline{\mathtt{filter}}(t, X) =
\Bigg\{
  \begin{aligned}
    & \mathtt{filter}(t, X) 
      &&\mathtt{if}\; \mathtt{filter}(t, X) \neq \emptyset \\
    & \{ \mathtt{def} \}
      &&\mathtt{otherwise}
  \end{aligned} &&
\end{flalign*}
...
% \mathtt{mut_{r}}(C_i \; x_1 \; x_2 \; \mydots) = 
%   \{ \texttt{PURE}\, x_j \, | \, 
%   x_j \!\in\! \{ x_1, x_2, \mydots \}, \; 
%   \texttt{typeof}(x_j) = T \}
% \end{align*}

% \begin{align*}
% \mathtt{mut_{s}}(C_i \; x_1 \; x_2 \; \mydots) = 
%   \{ \texttt{PURE}\, (C_j \, x_1^\prime \, x_2^\prime \, \mydots) \, | \,
%   &C_j \!\in\! \{ C_1, C_2, \mydots \}, \, \\
%   &x_k^\prime \!\in\! \{ x_k \, | \, x_k \!\in\! \{ x_1, x_2, \mydots \}, \mathtt{typeof}(x_k) = t_k^j \}) \}
% \end{align*}
...
functions.
% that can be automatically synthesized.
%
...
%
% The rest of the \texttt{MutateInside}'s functionality simply unwraps and
% rewraps the intermediate subexpressions and is not particularly relevant for
...
case associated to a random mutation.
%
...
sporadically.
%
We realized that we can use this information to dynamically adapt the number of
random mutations used by our tool.
%
...
\caption{\label{algo:mutagen:reset}Adaptive Random Mutations Heuristic}
\end{algorithm}
...
versus the relative overhead of our custom code instrumentation.
...
weakening the checks stored in this rule table.
%
For instance, the following are the buggy rule variations (in {\color{red} red})
for the \texttt{Store} instruction:
...
\textbf{Instruction} & \textbf{Precondition Check} & \textbf{Final PC Label} & \textbf{Final Result Label} \\
\hline
\texttt{Store} & \color{red}{$l_{pc} \sqsubseteq l_{v}$} & $l_{pc}$ & $l_{v'} \vee l_{pc} \vee l_{p}$ \\
\texttt{Store} & \color{red}{$l_{p} \sqsubseteq l_{v}$} & $l_{pc}$ & $l_{v'} \vee l_{pc} \vee l_{p}$ \\
\hline
\texttt{Store} & $l_{pc} \vee l_{p} \sqsubseteq l_{v}$ & \color{red}{$\bot$} & $l_{v'} \vee l_{pc} \vee l_{p}$ \\
\hline
\texttt{Store} & $l_{pc} \vee l_{p} \sqsubseteq l_{v}$ & $l_{pc}$ & \color{red}{$l_{pc} \vee l_{p}$} \\
\texttt{Store} & $l_{pc} \vee l_{p} \sqsubseteq l_{v}$ & $l_{pc}$ & \color{red}{$l_{v'} \vee l_{p}$} \\
\texttt{Store} & $l_{pc} \vee l_{p} \sqsubseteq l_{v}$ & $l_{pc}$ & \color{red}{$l_{v'} \vee l_{pc}$} \\
\hline
\end{tabular}
\end{center}
...
%
% this engine had several latent bugs that were not caught by the
% existing testsuite and that we discovered by \mutagen while developing our test
% suite.
%
...
scenarios that require generating highly-structured inputs (valid IFC machine
states and WebAssembly programs). 
%
...
% However, we are highly confident that 
%
% In addition, one should also consider some  
% that the specific performance the
% runtime systems of these two programming languages can affect, the
% draw strong conclusions based only on time-to-failure.
...
%
\citeauthor{ClaessenDP14} \cite{ClaessenDP14} developed an algorithm for
generating inputs constrained by boolean preconditions with almost-uniform
distribution.
%
% Luck
%
Later, \citeauthor{LampropoulosGHH17} \cite{LampropoulosGHH17} extended
...
%
Recently, \citeauthor{Lampropoulos2017} \cite{Lampropoulos2017} proposed a
mechanism to obtain constrained random generators by defining their target
...
%are strict when executing test cases obtained by mutating valid existing ones,
%
