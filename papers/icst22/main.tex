...
\title{
  \mutagen: Coverage-Guided, Property-Based Testing using
  Exhaustive Structure-Preserving Mutations
}

\author{\IEEEauthorblockN{1\textsuperscript{st} Agust\'in Mista}
  \IEEEauthorblockA{
    ...
    mista@chalmers.se}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Alejandro Russo}
\IEEEauthorblockA{
  \textit{dept. name of organization (of Aff.)} \\
  \textit{Chalmers University of Technology}\\
  Gothenburg, Sweden \\
...
codebase.
%
Randomly generating valid programs to test compilers is a common example of this
problem \cite{chen2020asurvey}, where developers are forced to put substantial
efforts in writing specialized random generators by hand \cite{Palka11,
  perenyi2020stack, yang2011finding}.
%
Moreover, automatically synthesized random generators offer poor results when
the testing properties are constrained by sparse preconditions.
%
...
%% %
%% Despite this limitation, \cite{lampropoulos2019coverage} show that
...
\footnote{Some of these were already identified by
  \cite{lampropoulos2019coverage} in their original work.}
%
of the original CGPT approach by \cite{lampropoulos2019coverage} that
...
%% %% testing.
%% %
%% Despite this, \cite{lampropoulos2019coverage} show that \fuzzchick is far
...
\cite{lampropoulos2019coverage} in their original work, and an existing
...
%
%% \cite{lampropoulos2019coverage} introduced \emph{coverage-guided,
...
\cite{honggfuzz}, or \emph{libFuzzer} \cite{libfuzzer} as well as
...
\cite{lampropoulos2019coverage} demonstrated empirically that CGPT lies
...
%% \cite{lampropoulos2019coverage}), \emph{\fuzzchick was only able to find 5
...
\cite{lampropoulos2019coverage} are derived to follow a top-down approach.
...
To keep the comparison fair, \cite{lampropoulos2019coverage} replicated
...
%
\footnote{An anonymized replication package \cite{anon_2021} is available for reviewing purposes.}
%
...
%
Its implementation was proven correct by \cite{10.1145/2578855.2535839}
\cite{10.1145/2578855.2535839} in Coq, and subsequently degraded by
systematically introducing 20 bugs on its enforcing mechanism.
%
\cite{lampropoulos2019coverage} used this case study to compare \fuzzchick
...
\cite{hritcu2013testing} \cite{hritcu2013testing,
  hrictcu2016testing} and \cite{lampropoulos2019coverage} for more details
...
%
\cite{lampropoulos2019coverage} demonstrated that generating two
...
\cite{lampropoulos2019coverage} and collected the mean-time-to-failure
...
the ones presented originally by \cite{lampropoulos2019coverage}, we
...
required by both tools to find the first failure \emph{when we only consider
  successful runs.}
%
Additionally, as advised by \cite{arcuri2014hitchhiker}
...
%
\cite{ClaessenDP14} \cite{ClaessenDP14} developed an algorithm for
generating inputs constrained by boolean preconditions with almost-uniform
distribution.
%
% Luck
%
Later, \cite{LampropoulosGHH17} \cite{LampropoulosGHH17} extended
...
%
Recently, \cite{Lampropoulos2017} \cite{Lampropoulos2017} proposed
