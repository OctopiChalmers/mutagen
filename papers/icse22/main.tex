...
In this section, we describe in detail the motivation, ideas and limitations
behind coverage-guided, property-based testing as depicted by
...
\noindent This definition follows a simple type-directed generation approach,
...
subexpression needed to produce a well-typed value, carefully reducing the depth
limit (controlled by \quickcheck) by a unit on each step, ensuring termination.
%
Subexpressions of any other type are generated simply by delegating the task to
their corresponding \texttt{Arbitrary} instance --- as noted by the use of
\texttt{arbitrary} when generating random payloads inside of the \texttt{Leaf}
and \texttt{Branch} constructors.

Readers familiar with RBPT will recognize that attempting to test this property
using \quickcheck directly does not work well.
%
In practice, \quickcheck will give up quite early if most of the generated test
cases get discarded because they do not satisfy the balanced precondition.
%
Sadly, most of the inputs generated by our na\"ive generator suffer from this
problem, and the interesting part of the property (\texttt{balanced (insert x t)})
...
The next subsection show how coverage-guided, property-based testing can help
overcoming this limitation without having to write specialized random generators
by hand.
...
while using automatically derived random generators,
\citeauthor{lampropoulos2019coverage} introduced \emph{coverage-guided,
  property-based testing} (CGPT), a technique that enhances the testing process
...
\item \emph{high-level, structure-preserving mutations}, to produce
  syntactically valid test cases by altering existing ones at the data type
  level.
\end{inparaenum}
...
%
CGPT, however, does this in a clever way.
%
...
%% The next subsections describe the CGPT testing loop and structure-preserving
%% mutations in detail.
...
queues, \textit{QSucc} and \textit{QDisc} for valid and discarded previously
...
\end{inparaenum}
%
We will provide more details about structure-preserving mutations in Section
\ref{sec:mutagen}.
...
%
\subsubsection{Limitations of CGPT}
...
(their reference implementation) and observed a surprising limitation:
%
...
improved upon --- and that constitute the main goal of this work.
%
...
%
the CGPT testing loop uses two single queues for keeping valid and discarded
mutation candidates.
%
...
``buried'' at the end of a long queue that only exercises the same portion of
the target code.
%
In an extreme case, they might not be processed at all within the testing
budget.
%
Ideally, \emph{one would like a mechanism that prioritizes mutating test cases
  that discover new portions of the code right away}, and that is capable of
...
%
it is not clear how the power schedule used to assign a given budget to each
mutable test case in CGPT works in the context of high-level
...
The next section introduces \mutagen, our CGPT tool that aims to tackle the main
limitations of CGPT.
...
In this section we describe the main ideas behind \mutagen, our enhanced CGPT
...
In contrast to CGPT, and many other popular fuzzing tools for that matter,
\mutagen works by to mutating test cases on an exhaustive and precise manner,
where
%
\begin{inparaenum}
...
%
Our realization is that, by following an exhaustive mutation approach, we can
avoid the need for a power schedule, i.e., a heuristic to assign a certain
mutation budget to each mutated test case.
%
...
produce test cases exhaustively --- please refer to Section \ref{sec:related}
...
%
As it can be observed, we follow the CGPT testing loop, using two queues to keep
mutable test cases, and enqueueing and retrieving interesting test cases from
them before falling back to random generation.
...
The difference with CGPT lies in that we precompute all the mutations of a given
mutable test case before enqueueing them.
%
...
Another difference is the criterion for enqueuing discarded tests.
%
...
%
\mutagen enables them all by default, and can be individually disabled by the
user if needed.
...
portions of the code under test, and hence we want to prioritize them.
