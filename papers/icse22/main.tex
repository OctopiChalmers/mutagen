...
sample a small number of values from a given random generator --- this amount is
a tunable parameter of \mutagen.
%
...
Having selected the next test case, the main loop proceeds to execute it,
capturing both the result (valid, discarded, or failed)
and its execution trace over the system under test.
%
...
if we set a shorter timeout and run it several times.
%
...
ideas to synthesize random generators from existing Haskell libraries and that
are used in tandem with existing low-level fuzzers to find bugs in heavily used
programs.
...
combining them with type-preserving mutations is part of our future work.
